...
function AddTodo() {
    ...
    const handleAddTodo = () => {
        if (todoValue.trim().length) {
            addTodo({ todoVal: todoValue, isEdit: false }, setTodoValue)
        }
    }
    return (
        <div>
            <div className="flex justify-center items-center w-full m-auto p-6">
                ...
                <button className='text-white font-bold text-2xl bg-addBtnBg rounded-md pt-1 pb-1 pl-2 pr-2 ml-2' onClick={handleAddTodo}><IoMdAdd /></button>
            </div>
        </div>
    )
}
